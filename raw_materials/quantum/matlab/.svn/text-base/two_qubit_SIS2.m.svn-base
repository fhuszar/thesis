function  [F index] = two_qubit_SIS2(true_case,selection)

% selection = 'MUB', 'aMUB', 'aSSQT', 'fSSQT'

%% initialise variables
dim = 4; %two qubit
k = 4; %full rank
S = 3000; % number of samples
Sthreshold = 500; % threshold, when ESS falls below this, resample
...
if strcmp(selection,'MUB')||strcmp(selection,'aMUB')
    measurements = Ms_MUB;
elseif strcmp(selection,'SSQT')||strcmp(selection,'aSSQT')
    measurements = Ms_SSQT;
elseif strcmp(selection,'fSSQT')
...
for iter = 1:Niters
    ...
    %initialise sigma
    %     sigma = 0.1; % try to come up with better way of initialising sigma
    ...
    for n=1:Nmax
        ...
        if strcmp(selection,'aMUB')||strcmp(selection,'aSSQT')||strcmp(selection,'fSSQT')
            % performing maximisation
            [expgain(n),index(n)] = max(h1-h2);
        elseif strcmp(selection,'MUB')||strcmp(selection,'SSQT')
            index(n) = mod(n,length(h1))+1;
        end
        ...
        F(n,iter) = real(weights'*fidelities/sumweights);
